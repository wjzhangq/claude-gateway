...
// stripThinkingSignatures removes the "signature" field from thinking/redacted_thinking
// content blocks in messages. Bedrock may reject signatures generated by a different
// model or inference profile.
func stripThinkingSignatures(messages []interface{}) []interface{} {
...
func prepareAnthropicBody(body []byte) []byte {
	...
	// Also strip invalid signatures from thinking blocks in messages.
			...
			if err := json.Unmarshal(raw, &parsed); err == nil {
				...
				if key == "messages" {
					if msgs, ok := cleaned.([]interface{}); ok {
						cleaned = stripThinkingSignatures(stripEmptyTextBlocks(msgs))
					}
				}
