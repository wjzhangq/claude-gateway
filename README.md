...
- **管理总览**：管理员仪表盘，实时查看系统级用户数、费用、请求量等关键指标
...
- **管理总览**：系统级仪表盘，一览总用户数、活跃用户、今日/本月费用、费用趋势
- **用户管理**：创建用户、修改角色/状态/配额，启用/禁用 AWS 渠道权限
- **申请审批**：审批或拒绝用户的模型使用申请
- **全局统计**：查看所有用户的用量数据，按日汇总费用
- **渠道统计**：按 Provider（Backend/AWS/Kimi/MiniMax）查看请求量和费用趋势
- **用户费用排行**：按日查看 Top 用户费用，识别高消耗用户
